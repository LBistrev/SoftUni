class PizzaDelivery:
    ...
    def make_order(self):
        if self.ordered:
            return f"Pizza {self.name} already prepared, and we can't make any changes!"
        self.ordered = True
        all_products = ', '.join([f"{key}: {value}" for key, value in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {all_products} and the price will be {self.price}lv."